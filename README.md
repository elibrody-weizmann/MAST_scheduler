...
1. **Filters** plans through a feasibility chain: astronomical night → time window → airmass → moon phase → moon separation → unit quorum → repeat quota. The airmass and moon-separation checks evaluate constraints at three checkpoints — start, mid-point, and end of the planned observation window (`requested_exposure_duration × requested_number_of_exposures`) — so a plan that sets below the horizon or enters a moon-exclusion zone before it finishes is rejected. Rejection rationales include `check_offset_seconds` and `check_label` to identify when in the window the constraint failed.
